...
\begin{align}
  \epsilon_g &= \int P(u, v) \cdot \epsilon(u,v)du dv \\
  &= \int dudv P(u,v)\Theta(-uv) \tha (\because \epsilon = |u-v|) \\
  &= \int_0^\infty du \int_{-\infty}^0 dvP(u,v) + \int_{-\infty}^0 du \int_0^\infty dv P(u,v) (\because u,v\text{どちらかが負の場合だけ発火}) \\
  &= \int_0^\infty du \int_{-\infty}^0 dvP(u,v) + \int_{\infty}^0 (-du) \int_0^{-\infty} (-dv)P(-u,-v) (\because u' = -u, v' = -v) \\
  &= \int_0^\infty du \int_{-\infty}^0 dvP(u,v) + \int_{\infty}^0 du \int_0^{-\infty} dv P(u,v) \\
  &= 2\int_0^\infty du \int_{-\infty}^0 dv P(u,v) \\
  &= 2\codt \frac{1}{2\pi \sqrt{1-R^2}} \int_0^\infty du \int_{-\infty}^0 dv \exp(-\frac{u^2+v^2 2Ruv}{2(1-R^2)}) \\
  &= \frac{1}{\pi \sqrt{1-R^2}} \int _0^\infty du \int_{-\infty}^0 dv \exp(-\frac{u^2 + (v-Ruv)^2 -R^2 u^2}{2(1\R^2)}) (\because u\text{のみの項を作りたい}) \\
  &= \frac{1}{\pi \sqrt{1-R^2}} \int_0^\infty du \exp(-\frac{u^2}{2}) \int_{-\infty}^0 dv \exp(-\frac{(v-Ru)^2}{2(1-R^2)}) \\
  &= \frac{1}{\pi \sqrt{1-R^2}} \int_0^\infty du \exp(-\frac{u^2}{2}) \int_{-\infty}^{-Ru} dw \exp(-\frac{w^2}{2(1-R^2)})\\
  &(\because w = v-Ru, dw = dv(u\text{は定数とみなせる}), v \to 0 \Rightarrow w \to -Ru)\\
  &= a
\end{align}
